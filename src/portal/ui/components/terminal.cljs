(ns portal.ui.components.terminal
...
(defn theme [settings]
  #js {:background (::c/background settings)
       :black (:nord1 nord)
       :blue (:nord9 nord)
...
(defn- make-terminal [settings value]
  (let [{:keys [on-stdin on-stdout on-resize]} value
        term (Terminal.
              #js {:convertEol true
                   :cursorBlink true
        ...
        #_stderr #_(fn []
                     (.then
                      (on-invoke (:on-stderr value))
                      (fn [data] (.write term data))))
        resize    #(.fit fit-addon)]
    (.loadAddon term fit-addon)
    (.loadAddon term (WebLinksAddon.))
    (-> term
        (.onData
          (fn [data]
            (on-stdin (.encode (js/TextEncoder. "utf-8") data)))))
    (-> term
        (.onBinary
          (fn [data]
            (on-stdin (.encode (js/TextEncoder. "binary") data)))))
    (stdout)
    (.onResize term #(on-resize (.-cols term) (.-rows term)))
    {:component-did-mount
     (fn [this]
